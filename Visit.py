...
while True:
     ...
     if posts.messages[0].message.find("No link promotions found for you right now.. Why don't you create one?") != -1:
         print(f"\n{abu2}[{red2}x{abu2}] {red}Iklan Sudah Habis Coba Lagi Besok")
         break
     try:
       print ("Visit To "+posts.messages[0].reply_markup.rows[0].buttons[0].url)
       r = c.get(posts.messages[0].reply_markup.rows[0].buttons[0].url)
       soup = BeautifulSoup(r.text,"html.parser")
       if soup.find("i", id="timer") is not None:
         tunggu(int(soup.find("i", id="timer").text))
         for val in soup.findAll("input"):
               link = (val.get("value"))
               c.get("https://doge.alexdev.it/link.php?u="+link)
