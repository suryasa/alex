...
\____/\___/_/ /\__,_/_/|_|\__,_/
         /___/                   """+Style.DIM+Fore.WHITE+"""Join Channel
...
for i in range(5000000):
    ...
    posts = client(
        GetHistoryRequest(peer=channel_entity, limit=1, offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0,
                          hash=0))
    if posts.messages[0].message.find(" No Channel Promotions found for you right now.. Why don't you create one?") != -1:
        print(f"\n{abu2}[{red2}x{abu2}] {red}Iklan Sudah Habis Coba Lagi Besok")
        break
    url = posts.messages[0].reply_markup.rows[0].buttons[0].url
    sys.stdout.write("\r")
    sys.stdout.write(f"\r{abu2}[{yellow2}!{abu2}]{yellow} Join Ke " + url)
    sys.stdout.flush()
    id = posts.messages[0].id
    client(JoinChannelRequest(url))
    try:
        client(
            GetBotCallbackAnswerRequest(channel_username, id, data=posts.messages[0].reply_markup.rows[1].buttons[1].data))
    except:
        sleep(3)
        posts = client(
            GetHistoryRequest(peer=channel_entity, limit=1, offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0,
                              hash=0))
        sys.stdout.write(f"\r{hijau}"+posts.messages[0].message)
        client.send_message(entity=channel_entity, message="Back 🔙")
